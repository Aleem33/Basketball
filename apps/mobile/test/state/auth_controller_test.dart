...
void main() {
  test(
    'authentication controller exposes failures without a fake session',
    () async {
      final MockAuthRepository repository = MockAuthRepository();
      when(
        () => repository.login('person@example.com', 'invalid'),
  ...
  test(
    'authentication controller stores an authenticated user in state',
    () async {
      final MockAuthRepository repository = MockAuthRepository();
      ...
      when(
        () => repository.login('person@example.com', 'valid-password'),
