...
class FavoritesState {
  const FavoritesState({this.teamIds = const <String>{}, this.tournamentIds = const <String>{}, this.isOffline = false});
...
class FavoritesRepository {
  ...
  Future<FavoritesState> fetch() async {
    final Map<String, Object?> data = await _api.get<Map<String, Object?>>('/me/favorites', (Object? raw) => (raw! as Map).cast<String, Object?>());
    final Set<String> teams = (data['teams']! as List<Object?>).map((Object? row) => (row! as Map)['id']! as String).toSet();
    final Set<String> tournaments = (data['tournaments']! as List<Object?>).map((Object? row) => (row! as Map)['id']! as String).toSet();
    await _preferences.setString('favorite_team_ids', jsonEncode(teams.toList()));
    await _preferences.setString('favorite_tournament_ids', jsonEncode(tournaments.toList()));
    return FavoritesState(teamIds: teams, tournamentIds: tournaments);
  }

  Future<void> setTeam(String id, bool favorite) async {
    if (favorite) {
      await _api.put<void>('/me/favorites/teams/$id', const <String, Object?>{}, (Object? _) {});
    } else {
  ...
  Future<void> setTournament(String id, bool favorite) async {
    if (favorite) {
      await _api.put<void>('/me/favorites/tournaments/$id', const <String, Object?>{}, (Object? _) {});
    } else {
