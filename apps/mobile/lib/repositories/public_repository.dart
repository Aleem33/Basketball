...
class PublicRepository {
  ...
  Future<CachedResult<List<TournamentSummary>>> tournaments({String? search}) async {
    try {
      final List<TournamentSummary> result = await _api.get<List<TournamentSummary>>(
        '/public/tournaments',
        (Object? raw) {
          final Map<String, Object?> page = (raw! as Map).cast<String, Object?>();
          return (page['items']! as List<Object?>)
              .map((Object? item) => TournamentSummary.fromJson((item! as Map).cast<String, Object?>()))
              .toList(growable: false);
        },
        query: <String, Object?>{if (search != null && search.isNotEmpty) 'search': search},
      );
      await _writeCache('tournaments', result.map((TournamentSummary item) => <String, Object?>{
        'id': item.id,
        'name': item.name,
        'slug': item.slug,
      ...
      return CachedResult<List<TournamentSummary>>(
        cached.map((Object? item) => TournamentSummary.fromJson((item! as Map).cast<String, Object?>())).toList(),
        isStale: true,
      );
    }
  }

  Future<CachedResult<List<GameSummary>>> games({GameStatus? status, String? tournamentId}) async {
    try {
      final List<GameSummary> result = await _api.get<List<GameSummary>>(
        ...
        (Object? raw) {
          final Map<String, Object?> page = (raw! as Map).cast<String, Object?>();
          return (page['items']! as List<Object?>)
              .map((Object? item) => GameSummary.fromJson((item! as Map).cast<String, Object?>()))
              .toList(growable: false);
        },
        query: <String, Object?>{
      ...
      return CachedResult<List<GameSummary>>(
        cached.map((Object? item) => GameSummary.fromJson((item! as Map).cast<String, Object?>())).toList(),
        isStale: true,
      );
    }
  }

  Future<Map<String, Object?>> tournament(String id) => _api.get<Map<String, Object?>>(
    '/public/tournaments/$id',
    (Object? raw) => (raw! as Map).cast<String, Object?>(),
  );

  Future<Map<String, Object?>> team(String id) => _api.get<Map<String, Object?>>(
    '/public/teams/$id',
    (Object? raw) => (raw! as Map).cast<String, Object?>(),
  );

  Future<Map<String, Object?>> game(String id) => _api.get<Map<String, Object?>>(
    '/public/games/$id',
    (Object? raw) => (raw! as Map).cast<String, Object?>(),
  );

  Future<Map<String, Object?>> standings(String stageId) => _api.get<Map<String, Object?>>(
    '/public/stages/$stageId/standings',
    (Object? raw) => (raw! as Map).cast<String, Object?>(),
  );

  Future<Map<String, Object?>> bracket(String stageId) => _api.get<Map<String, Object?>>(
    '/public/stages/$stageId/bracket',
    (Object? raw) => (raw! as Map).cast<String, Object?>(),
  );

  Future<Map<String, Object?>> search(String query) => _api.get<Map<String, Object?>>(
    '/public/search',
    (Object? raw) => (raw! as Map).cast<String, Object?>(),
  ...
  Future<void> _writeCache(String key, Object value) async {
    await _preferences.setString('cache_$key', jsonEncode(<String, Object?>{
  ...
  List<Object?>? _readCache(String key) {
    ...
    final Map<String, Object?> data = (jsonDecode(encoded) as Map).cast<String, Object?>();
    final DateTime storedAt = DateTime.parse(data['storedAt']! as String);
    if (DateTime.now().toUtc().difference(storedAt) > const Duration(hours: 24)) return null;
