...
class HonestEmptyState extends StatelessWidget {
  const HonestEmptyState({required this.title, required this.message, super.key});
  final String title;
...
class FailureState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final String message = error is AppFailure ? (error as AppFailure).message : 'Something went wrong. Try again.';
    final IconData icon = error is NetworkFailure ? Icons.cloud_off_outlined : Icons.error_outline;
    return Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: <Widget>[
      Icon(icon, size: 44), const SizedBox(height: 12), Text(message, textAlign: TextAlign.center), const SizedBox(height: 16),
...
class OfflineBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialBanner(
    ...
    actions: <Widget>[TextButton(onPressed: ScaffoldMessenger.of(context).hideCurrentMaterialBanner, child: const Text('Dismiss'))],
  );
}
